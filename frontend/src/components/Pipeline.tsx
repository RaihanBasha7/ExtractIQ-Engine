...
export function PipelineTimeline({ stages, activeIndex, inRepair = false, errorStage = null }: PipelineTimelineProps) {
  return (
        ...
        return (
          <div
            ...
            className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 border transition-all duration-300 ${
              isError
                ? 'bg-red-500/10 border-red-500/40 shadow-[0_0_30px_-6px_rgba(239,68,68,0.5)]'
                : isActive
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-medium ${
                  isError ? 'text-red-300' : isActive ? 'text-white' : isDone ? 'text-white/70' : 'text-white/40'
                }`}
              >
                {stage.label}
                {isRepair && inRepair && (
            ...
            {i < stages.length - 1 && (
              <div className={`absolute left-[27px] top-full w-px h-2.5 ${isError ? 'bg-red-500/40' : 'bg-white/10'}`} />
            )}
          </div>
        );
      })}
...
export function NodeFlow({ activeIndex, inRepair = false, completed = false, failed = false, errorStage = null }: NodeFlowProps) {
  const active = nodeActiveAt(activeIndex);
  const isRunning = activeIndex >= 0 && activeIndex < PIPELINE_STAGES.length;
  const errorNodes = errorStage != null ? nodeActiveAt(errorStage) : [];
    ...
    <div className="relative">
      <div className="relative flex flex-col gap-3">
        {NODES.map((node, i) => {
          ...
          const isError = failed && errorNodes.includes(node.id);
            ...
            <div key={node.id} className="relative">
              <div
                className={`relative flex items-center gap-3 rounded-2xl px-4 py-3 border bg-white/[0.02] transition-all duration-400 ${
                  isError
                    ? 'border-red-500/50 shadow-[0_0_30px_-6px_rgba(239,68,68,0.5)]'
                    : isActive
                      ? completed
              ...
              >
                ...
                <div className="flex-1">
                  <p className={`text-sm font-semibold ${isError ? 'text-red-300' : isActive ? 'text-white' : 'text-white/55'}`}>
                    {node.label}
                  </p>
                </div>
              ...
              {i < NODES.length - 1 && (
                <div className="flex justify-center py-1">
                  <div className={`relative w-px h-5 ${isError ? 'bg-red-500/40' : 'bg-white/10'} overflow-hidden`}>
                    {isRunning && active.includes(node.id) && !completed && !failed && (
                      <span
                    ...
                    {isError && (
                      <span className="absolute left-0 top-0 w-px h-full bg-red-500" />
                    )}
                  </div>
                </div>
              )}
