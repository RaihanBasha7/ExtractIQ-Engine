...
export const Navbar = memo(function Navbar({ onMenu, scrolled }: NavbarProps) {
  ...
  return (
    <header className="relative z-20" ref={headerRef}>
      <div
        className={`relative rounded-2xl border px-4 py-2.5 flex items-center gap-2.5 transition-all duration-500 ease-out overflow-hidden ${
          scrolled
            ? 'bg-[rgba(13,19,34,0.82)] backdrop-blur-2xl border-[rgba(255,255,255,0.14)] shadow-[0_12px_40px_-12px_rgba(0,0,0,0.7),0_0_0_1px_rgba(255,255,255,0.06)]'
            : 'bg-[rgba(13,19,34,0.40)] backdrop-blur-xl border-[rgba(255,255,255,0.08)] shadow-[0_8px_30px_-14px_rgba(0,0,0,0.5)]'
        } dark:bg-[rgba(13,19,34,0.82)] dark:backdrop-blur-2xl dark:border-[rgba(255,255,255,0.14)]`}
      >
        ...
        <div className="hidden md:flex items-center gap-2 rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-1.5 w-56 lg:w-64 focus-within:border-brand-blue/50 focus-within:shadow-[0_0_20px_-6px_rgba(59,130,246,0.3)] transition-all duration-300">
          <Search size={14} className="text-white/40 group-focus-within:text-brand-blue transition-colors" />
          <input
          ...
          />
          <kbd className="text-[10px] text-white/30 border border-white/10 rounded px-1.5 py-0.5 font-mono">⌘K</kbd>
        </div>
