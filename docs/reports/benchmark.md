...
**Important caveat:** These measurements are from a single evaluation run. A larger benchmark (75+ tickets) is needed for statistically significant percentiles. See `backend/reports/evaluation_results_full.json` for raw data.
...
*Generated by the ExtractIQ Engine evaluation harness on 2026-07-20. Full results available in `backend/reports/evaluation_results_full.json`. Metrics marked "Not Yet Measured" require pipeline enhancements described above.*
