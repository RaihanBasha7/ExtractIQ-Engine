...
class TestExtractionRepository:
    ...
    def test_count_extractions(self, db_session: Session):
        ...
        repo.save(ticket_id="TKT-001", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=0.1, db_session=db_session)
        repo.save(ticket_id="TKT-002", structured_json=None, schema_valid=False, retry_count=3, failure_category="validation_error", latency_seconds=0.5, db_session=db_session)
    ...
    def test_successful_count(self, db_session: Session):
        ...
        repo = ExtractionRepository()
        repo.save(ticket_id="TKT-001", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=0.1, db_session=db_session)
        repo.save(ticket_id="TKT-002", structured_json=None, schema_valid=False, retry_count=0, failure_category="error", latency_seconds=0.1, db_session=db_session)
...
class TestMetricsAggregation:
    def test_average_retry_count(self, db_session: Session):
        ...
        repo.save(ticket_id="TKT-001", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=0.1, db_session=db_session)
        repo.save(ticket_id="TKT-002", structured_json=None, schema_valid=True, retry_count=2, failure_category=None, latency_seconds=0.2, db_session=db_session)
    ...
    def test_average_processing_time(self, db_session: Session):
        ...
        repo = ExtractionRepository()
        repo.save(ticket_id="TKT-001", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=1.0, db_session=db_session)
        repo.save(ticket_id="TKT-002", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=3.0, db_session=db_session)
    ...
    def test_failure_breakdown(self, db_session: Session):
        ...
        repo = ExtractionRepository()
        repo.save(ticket_id="TKT-001", structured_json=None, schema_valid=False, retry_count=3, failure_category="validation_error", latency_seconds=0.5, db_session=db_session)
        repo.save(ticket_id="TKT-002", structured_json=None, schema_valid=False, retry_count=2, failure_category="validation_error", latency_seconds=0.3, db_session=db_session)
        repo.save(ticket_id="TKT-003", structured_json=None, schema_valid=False, retry_count=1, failure_category="timeout", latency_seconds=0.1, db_session=db_session)
    ...
    def test_get_stats(self, db_session: Session):
        ...
        repo = ExtractionRepository()
        repo.save(ticket_id="TKT-001", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=0.5, db_session=db_session)
        repo.save(ticket_id="TKT-002", structured_json=None, schema_valid=False, retry_count=2, failure_category="error", latency_seconds=1.5, db_session=db_session)
...
class TestHistoryRetrieval:
    ...
    def test_list_history_with_data(self, db_session: Session):
        ...
        ext_repo.save(ticket_id="TKT-001", structured_json={"key": "val"}, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=0.5, db_session=db_session)
    ...
    def test_history_pagination(self, db_session: Session):
        ...
        for i in range(5):
            raw_repo.save(ticket_id=f"TKT-{i:03d}", raw_text=f"Text {i}", cleaned_text=f"Text {i}", language="en", db_session=db_session)
            ext_repo.save(ticket_id=f"TKT-{i:03d}", structured_json=None, schema_valid=True, retry_count=0, failure_category=None, latency_seconds=0.1, db_session=db_session)
